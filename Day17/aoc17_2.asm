...
_start:
    ...
    mov r8,  0                      ; A (R8 ) register (not needed here)
    mov rcx, 0                      ; actual start (add 1) value of A register, where search will start

    mov r9,  0                      ; B (R9 ) register
    mov r10, 0                      ; B (R10) register

;; functionality for initializing program with next A ;;
program_init:
    ; initialize program

    inc rcx                         ; increment the start value of the A register
    mov r8, rcx                     ; initialize the A register

    mov rdx, rcx                    ; for overview where program is: printing current iteration
    and rdx, 0xFFFFF                ; mask the last 20bits. (~1.000.000)
    test rdx, rdx                   ; check if result is zero
    jnz program_init_no_print       ; if not equal: skip printing
    mov rdx, rcx
    push rcx                        ; safe rcx on the stack
    call print_rdx
    pop rcx                         ; load rcx from the stack

program_init_no_print:
    mov rdi, 0                      ; number of correct output digits
...
program_check_exit:
    ...
    jmp program_init                ; else try next A register
...
out:
    ...
    jge program_init                ; if greater: reinitialize program

    xor rax, rax
    mov al, byte [program + edi]
    cmp dl, al                      ; compare rdx with value in program[rdi]
    jne program_init                ; if not equal: reinitialize program
    inc rdi
...
exit:
    ...
    mov rdx, rcx                    ; print start contents of A register
    call print_rdx
